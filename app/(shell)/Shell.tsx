...
export default function Shell({ children }:{ children: ReactNode }) {
  const s = useSession();                          // ← ここを分割代入しない
  const role = (s?.data as any)?.role ?? "client"; // ← undefinedでもOKに
  ...
  return (
    <div className="min-h-screen flex">
      <aside className="w-64 bg-white border-r flex flex-col">
        <div className="h-14 flex items-center px-4 text-lg font-semibold border-b">
          ADMaster <span className="ml-2 text-xs text-gray-500">{role === "admin" ? "管理者" : "店舗A"}</span>
        </div>
          ...
          {menu.map((m) => (
            <Link
              ...
              className={cn(
                ...
                pathname === m.href ? "bg-gray-900 text-white" : "text-gray-700 hover:bg-gray-100"
              )}
            >
              {m.label}
            </Link>
